---
title: 前端代码规范
date: '2018-10-02'
spoiler: 关于前端代码规范的资料收集，让我们的项目如丝般顺滑。
---

# 前端代码规范

## 代码规范管理

- [📏 使用 ESLint 来管理你的 git commit 代码规范](https://github.com/zhanghao-zhoushan/use-eslint-in-git-hooks)

## HTML 和 CSS 代码风格规范

- [编码规范 by @mdo](https://codeguide.bootcss.com)

- [Code Guide by @mdo](http://codeguide.co)

### 模块化 CSS: BEM

- [BEM 的定义](https://www.w3cplus.com/css/bem-definitions.html)

- [编写模块化 CSS：BEM](https://www.w3cplus.com/css/css-architecture-1.html)

## JavaScript 代码风格规范

- [Airbnb JavaScript Style Guide](https://github.com/airbnb/javascript/blob/master/README.md)

- [JavaScript Standard Style](https://standardjs.com/readme-zhcn.html)

- [JavaScript Standard Style GitHub](https://github.com/standard/standard/blob/master/docs/README-zhcn.md)

- [JavaScript 编码规范 百度](https://github.com/ecomfe/spec/blob/master/javascript-style-guide.md)

## Vue 代码风格规范

- [风格指南 — Vue.js](https://cn.vuejs.org/v2/style-guide/)

## React 代码风格规范

- [Airbnb React/JSX Style Guide](https://github.com/airbnb/javascript/tree/master/react)

## AMD CMD 规范

- [Asynchronous Module Definition (AMD) ](https://github.com/amdjs/amdjs-api/wiki/AMD)

- [CMD 模块定义规范 sea.js](https://github.com/seajs/seajs/issues/242)

## 文档规范

- [中文技术文档的写作规范 阮一峰](https://github.com/ruanyf/document-style-guide/blob/master/README.md)
